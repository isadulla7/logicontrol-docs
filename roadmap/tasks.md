...
| `WB-13` | web | **W1/W2 maket tartibi**: holat filtri, saralash izohi, klaviatura navbat rejimi (↑↓/Enter/A/R/Esc), fokusli qator, detal panel (FX surati, offline farq), qarordan keyin keyingisi. | `WB-12`, `DS-03` |
| `WB-14` | web | **Operator kirish (W-L) va kompaniya onboarding (W-O)**: kontrakt §3a auth, sessiya konteksti, STIR reestr autofill, ildiz marshruti yo'naltirishi (OPEN-011). | `DC-03 §3a`, `DS-04`, `WB-13` |
| `WB-15` | web | **DS-05 xato matnlari katalogini ulash** (`design/copy/error-codes.md`, 48 kod uz+ru): xato ekranlari texnik kod o'rniga inson matnini ko'rsatsin, kod + correlation ID texnik satrda qolsin. | `DS-05`, `WB-14` |
...
| `WB-13` navbat rejimi va detal panel | ✅ DONE — holat filtri kontrakt `?status` bilan (berilmasa hamma holat); navbat kiritilgan vaqt bo'yicha, eskisi birinchi; klaviatura rejimi ↑↓/Enter/A/R/Esc (`aria-keyshortcuts`, matn kiritishda o'chadi); fokusli qator markeri; detal panel — yirik asl summa + bazaviy ekvivalent, muzlatilgan FX surati, kiritilgan/ofisga yetgan vaqtlar alohida, 2 soatdan katta farqda offline belgisi (OPEN-016); qarordan keyin keyingi yozuv o'zi ochiladi; panel ochiqda takrorlanuvchi ustun olib turiladi. Gate: lint+typecheck+85 test+build yashil. Eslatma: maketdagi Haydovchi/Tur/Sana/Summa filtrlari DC-03 da yo'q — kontrakt kengaytmasi kerak (`OPEN-025`) |
| `WB-14` operator kirish va onboarding | ✅ DONE — kontrakt v1.1 §3a konsolga ulandi: 24 soatlik sessiya, noto'g'ri email ham parol ham bitta neytral `AUTH_LOGIN_FAILED` (hisob mavjudligi oshkor emas), 5 xatodan keyin 429 `AUTH_RATE_LIMITED` (ADR-002), sessiyasiz `/operator/me` → 401, parol tiklash so'rovi har doim 200. `/login` W-L maketida (brend paneli yo'l motivi bilan, xatodan keyin parol tozalanadi), `/onboarding` W-O maketida — STIR reestri **boyitish manbai**: 9 raqam validatsiyasi, topilmasa xato emas `found:false`, «faoliyati to'xtatilgan» holati bloklamaydi; bazaviy valyutaning o'zgarmasligi ogohlantiriladi. Ildiz marshruti: sessiyasiz → `/login`; 0 kompaniya → `/onboarding`; aynan 1 ta → to'g'ridan `/c/{id}` (OPEN-011); ko'p bo'lsa tanlash. Token `localStorage` da, sahifa yangilanganda `/operator/me` orqali tiklanadi, 401 da tozalanadi. Gate: lint+typecheck+101 test+build yashil |
| `WB-15` xato matnlari katalogi | ✅ DONE — DS-05 (`design/copy/error-codes.md`) konsolga ulandi: `src/lib/i18n/error-codes.ts` da 44 kodning uz+ru matni, noma'lum kod uchun zaxira matn (§9), server `detail` i hech qachon ekranga chiqmaydi. Xato ekrani endi inson matnini birinchi ko'rsatadi, kod + correlation ID texnik qatorda qoladi. Kutish vaqti faqat serverdan: `retryAfterSeconds` problem+json dan o'qiladi, kelmasa matn vaqtsiz variantga tushadi (klient raqam o'ylab topmaydi). `disabledReason.code` ham shu katalogdan matn oladi (§11) — manba bitta. Ekran-ichi ad-hoc «Xatolik yuz berdi (kod: X)» satrlari olib tashlandi (navbat, reys, flot, hisob-kitob, ledger, kirish); ledger stornosining jim xatosi endi sabab aytadi. Gate: lint+typecheck+116 test+build yashil. Topilma — `OPEN-026` (uz ablativ qo'shimchasi) |
