...
for %%I in ("%~dp0.") do set "PLUGIN=%%~fI"
for %%I in ("%PLUGIN%\..") do set "DOCS=%%~fI"
...
if not exist "%DOCS%\.git" (
  echo [ERROR] logicontrol-docs repository not found: %DOCS%
  exit /b 1
)
if not exist "%BACKEND%\.git" (
  echo [ERROR] logicontrol-backend repository not found: %BACKEND%
  exit /b 1
)
if not exist "%ANDROID%\.git" (
  echo [ERROR] logicontrol-android repository not found: %ANDROID%
  exit /b 1
)
