...
`[D]` **Which bulk operations exist**, grounded in what canon supports:

| Workspace | Bulk operations `[D]` | Never |
|---|---|---|
| Alerts | acknowledge, assign, resolve `[C]` lifecycle is `OPEN → ACKNOWLEDGED → RESOLVED` | — |
| Expense approvals | approve, reject `[C]` reject requires a mandatory reason — so bulk reject collects one reason for the batch and the UI says it applies to all | — |
| Compliance | request renewal, assign owner | — |
| Members | suspend, reactivate | bulk role change |
| Ledger, Settlements | **none** | `[C]` append-only; no bulk anything |
| Audit | export only | `[C]` append-only |
