...
| **Motion** | `[C]` The driver brief calls for "no distracting motion"; the web brief additionally forbids scroll-linked motion in a hours-long tool ([07](07-responsive-behavior.md) § 5). Same conclusion, different reasons — so state the rule per platform rather than pretending it is one shared token. |
