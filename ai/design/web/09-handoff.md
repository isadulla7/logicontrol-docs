...
|---|---|---|---|
| 1 | Navigation mirrors module ownership; nine rail items in three groups | [02](02-information-architecture.md) §§ 1, 3 | `[DERIVED]` for the principle — and read the inference in [02](02-information-architecture.md) § 1, because it is narrower than it looks: canon **permits** a backend adapter to compose from two modules public APIs. What it forbids is cross-module coupling and the object graph. Module-shaped navigation is the arrangement that never tempts a screen into demanding the graph. `[D]` for the nine items and their grouping |
| 2 | Company scope is explicit in the URL, and is an addressing device, never a security mechanism | [02](02-information-architecture.md) § 4 | `[D]` for the URL shape; `[C]` that it confers no authorization — stated in `domain/GLOSSARY.md` § Tenancy and identity and `architecture/system-architecture-uz.md` § Multi-tenancy/security |
| 3 | All list state lives in the URL | [02](02-information-architecture.md) § 4 | `[D]` |
| 4 | Hub screens load related data as independent panels; no cross-module object graph | [02](02-information-architecture.md) § 5 | `[DERIVED]` — **not** from "composition is forbidden", which canon does not say. From two things it does say: a Trip does not own Expense/Fuel/WorkOrder/Document collections, and dashboards and P&L do not load the transaction graph. A hub designed as one object graph pushes against both. `[D]` independently: panels want their own loading, failure and permission states |
...
| 13 | Bulk operations are N idempotent per-item calls with per-item results; "select all matching" is a separate act; no bulk delete | [04](04-operational-patterns.md) § 10 | `[D]` for the pattern. On no-bulk-delete the warrant splits: `[DERIVED]` for Ledger, Settlement, Audit, Trip, WorkOrder and Expense, where canon gives append-only data or a lifecycle cancellation state, so a delete has nothing correct to do. `[?]` for master data — Vehicle, Driver and Customer each carry a `status`, and canon says nothing about deleting them; silence is not prohibition. The design still offers no bulk delete anywhere, but for master data that is `[D]`, not a canonical consequence |
...
- For the `[DERIVED]` warrants — 1, 4, 6, 8, 15 and the derived halves of 5, 7, 9 and 13 — "why" is
