...
| **Signing the driver out on any failed request.** | Today the client cannot tell an expired session from a captive portal from a 500, because no authentication code exists (**FACT** F-13, F-14). Guessing "expired" destroys a working session on a bad network. |
