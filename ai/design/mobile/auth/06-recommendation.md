...
| **D-04 local factor** | **C — biometric with app-local PIN fallback**, fallback reachable immediately | **DERIVED.** An earlier draft called this "the only option that survives PF-02 and PF-03", which was wrong: option B (PIN only) survives both equally — `05` D-04 records that it *"works on every device"*. C is chosen because it is the only option that survives **PLATFORM** PF-02 (no sensor or enrolment) and PF-03 (enrolment change) **and** does not make the driver type on every launch. The platform facts rule out A and D; ranking C above B is the designer's judgement about typing cost, not a platform constraint. |
...
| **D-07 session lifetime** | **B — long session, renewed opportunistically** on requests the app was making anyway | **DERIVED.** **PLATFORM** PF-07 states that OEM battery managers, Doze and App Standby can defer background work indefinitely. That a scheduled refresh is therefore an unreliable foundation for a *security* clock is the designer's inference from it, not something PF-07 says. |
...
| **D-10 sign-out** | **C — send first, then sign out; offline sign-out requires a destructive confirmation naming the count** | **DERIVED.** **FACT** F-23 forbids sign-out *silently* discarding accepted work, and rules out option A. It does **not** select C: options B (refuse while queued) and D (retain the queue) also satisfy it. Choosing C among the three compliant options — making the safe path the easy one and the lossy path deliberate — is the designer's judgement, not F-23's requirement. |
...
| **D-12 recovery** | **C — both**, with operator re-issuance as the primary route | **DERIVED.** An earlier draft said operator re-issuance was "the only route that survives a lost handset". That is false, and it contradicts `05` D-12 option A, which is precise: self-service fails if the phone **or the number** is lost. Under the recommended phone-number identifier (D-01), a driver who replaces the handset and keeps the SIM recovers by self-service OTP with no operator involved at all. Operator re-issuance is primary because it is the only route surviving the cases self-service cannot: a lost or changed number, a number unreachable at the moment it is needed (roaming, no signal — **FACT** F-19 makes that ordinary rather than exceptional), and a driver blocked at `AUTH-09`, where the self-service channel is itself what is blocked. A biometric key invalidated by **PLATFORM** PF-03 was in this list in an earlier draft and does not belong: that driver needs a new code like any other, and a reachable number delivers one with no operator involved. It needs an operator only in combination with the first two cases, which already cover it. Which of these is *commonest* is unmeasured, and this package no longer claims to know. `AUTH-10` must work offline. |
| **D-13 code channel** | **SMS primary, operator voice as the standing fallback. Push is not a primary channel.** | **DERIVED.** **FACT** F-29 names non-GMS devices as an open limitation in this market and defers handling it until measured; that push therefore cannot be a *primary* authentication channel is the inference, not F-29's statement. Two further steps are the designer's: that push needs a data connection, which is what a stranded driver lacks (**PLATFORM**, uncontroversial), and that voice reaches a driver who has signal but no data. |
